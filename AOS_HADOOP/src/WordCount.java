...
public class WordCount {
	public static class State implements WritableComparable<State> {

    String state;
    String gender;

    public State(String state, String gender) {
        this.state = state;
        this.gender = gender;
    }
	...
	public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
		 		if (((IntWritable) key).get() > 0 )
                    {
                        String line = value.toString();
                        String[] elements = line.split(",");

                        State state2 = new State(elements[2],elements[6]);
         
                
                        IntWritable it = new IntWritable(1);
                        context.write(new Text(state2.toString()), it);
                    }
